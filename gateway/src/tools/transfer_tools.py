...
the jadx-upload CLI), which relays it to the Java backend; the AI then calls
...
def register_transfer_tools(mcp):
    ...
    async def create_transfer_token(
        ...
        Workflow:
          ...
          2. Tell the user to run: `jadx-upload --upload-url <upload_url> --token <token> <local-file>`
