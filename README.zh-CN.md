...
docker compose pull        # 从 GHCR 拉取预构建镜像（省略则本地构建）
docker compose up -d       # 暴露 127.0.0.1:8651
...
把这个仓库（或就这一节）丢给 AI 编码助手，说一句*「给我装上 delamain 并接入」*，
助手照这份确定性 SOP 执行即可。

**目标：** 一个跑在 `127.0.0.1:8651` 的 delamain 容器，并把调用方的 MCP 客户端接上它。

1. **前置条件** —— 确认 Docker 在运行（`docker info`），并选一个存放待分析
   APK/DEX/AAB 的**绝对路径**目录（例如 `~/apks`）。
2. **生成 MCP token**（客户端的 bearer 密钥，请保存）：
   ```bash
   export DELAMAIN_AUTH_TOKENS="$(openssl rand -hex 32)"
   ```
3. **运行已发布镜像**（建议钉住版本 tag 而非 `latest` 以保证可复现）：
   ```bash
   ...
   ```
4. **验证健康**（重试到 `status:healthy`）：
   ```bash
   curl -s http://127.0.0.1:8651/health
   # → {"status":"healthy","version":"…","jadx_version":"1.5.6"}
   ```
5. **接入 MCP 客户端** —— streamable-HTTP 端点 + bearer token：
   - 端点：`http://127.0.0.1:8651/mcp`
   - 请求头：`Authorization: Bearer <DELAMAIN_AUTH_TOKENS 的值>`

   客户端配置示例（按你的 MCP 客户端格式调整）：
   ```json
   {
     "mcpServers": {
       "delamain": {
   ...
   ```
6. **确认工具可用** —— 未认证访问 `/mcp` 必须返回 `401`；带认证的 `tools/list` 必须返回工具集。
7. **第一次分析** —— 把 APK 放进挂载目录，然后经 MCP 调用
   `load_file_tool(path="app.apk")`，轮询 `get_decompile_status()` 到就绪，再用
   `search_classes_by_keyword`、`get_class_source`、`get_xrefs` 等。

**排障**
- `DELAMAIN_AUTH_TOKENS must be set` → 第 2 步没有 export 进 `docker run`。
- 端口 8651 被占用 → 改映射（`-p 127.0.0.1:9651:8651`）并同步改端点。
- `/health` 一直不健康 → 看 `docker logs delamain`；大 APK 在后台预热，重操作前先轮询 `get_warmup_status`。
- 每次调用都 `401` → 客户端的 bearer 与 `DELAMAIN_AUTH_TOKENS` 里的值不匹配。
