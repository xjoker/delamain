...
2. **The AI relays `token` and `upload_url` to the human**, who uploads the
   file using either curl or the `delamain-cli` CLI (see below).

3. **The client polls `GET /transfer/status`** (or just re-runs `delamain-cli`,
...
## Uploading: `delamain-cli` CLI (resumable, chunked, checksummed)

For large files, flaky connections, or when you want SHA-256 verification,
use the `delamain-cli` CLI in `tools/delamain-cli/` (Rust, cross-platform):

```sh
delamain-cli --server http://<gateway-host>:8651 --token <token> /path/to/app.apk
```

or, using the full `upload_url` returned by `create_transfer_token` directly:

```sh
delamain-cli --upload-url <upload_url> --token <token> /path/to/app.apk
```

Options (`tools/delamain-cli/README.md`):
