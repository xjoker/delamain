...
1. In your MCP client, call the `create_transfer_token` tool. It returns a
   ...
   `expires_at_epoch_ms`.
2. Run `jadx-upload` (below) with that token to push the file's bytes to
   the server.
...
and re-run `jadx-upload`; it will resume from wherever the previous attempt
left off as long as the token is still valid.

## Usage

```sh
jadx-upload --server http://host:8651 --token <token> /path/to/app.apk
```

Or, if you already have the full upload URL from `create_transfer_token`:

```sh
jadx-upload --upload-url http://host:8651/transfer/upload --token <token> /path/to/app.apk
```
...
The binary is produced at `target/release/jadx-upload`
(`target/release/jadx-upload.exe` on Windows).
...
Use `jadx-upload` instead whenever you want resumability, chunking for
